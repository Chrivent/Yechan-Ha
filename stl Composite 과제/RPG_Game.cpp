...
void RPG_Game::Dongeon()
{
	...
	while (true)
	{
		...
		case 2:
			...
			break;
		case 3:
			character = m_werewolf;
			break;
		case 4:
			character = m_ogre;
			break;
		case 5:
			character = m_skeletonArcher;
			break;
		case 6:
			character = m_rich;
			break;
...
void RPG_Game::Stage(Character* character)
{
	m_map.EraseBoxInner();
	m_map.DrawStage();
	m_player->DrawStatus(m_map.GetScale().width * 16, m_map.GetScale().height * 3);
	character->DrawStatus(m_map.GetScale().width * 16, m_map.GetScale().height * 25);
	...
	while (true)
	{
		...
		if (playerHand >= HAND_SCISSORS && playerHand <= HAND_PAPER)
		{
			...
			m_player->Event(m_map.GetScale().width * 16, m_map.GetScale().height * 14, character, result);
			m_player->DrawStatus(m_map.GetScale().width * 16, m_map.GetScale().height * 3);
			character->DrawStatus(m_map.GetScale().width * 16, m_map.GetScale().height * 25);
		}
		...
		if (m_player->GetHp() <= 0)
		{
			MatchResult(character, m_player);
			getch();
			break;
		}
		else if (character->GetHp() <= 0)
		{
			MatchResult(m_player, character);
			m_player->IncreaseGold(character->GetGold());
			getch();
			break;
		}
	}
