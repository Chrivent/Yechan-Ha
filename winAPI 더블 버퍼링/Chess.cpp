...
Tile* Chess::GetTileUnderMouse(LPARAM lParam)
{
	for (int y = 0; y < 8; y++)
	{
		for (int x = 0; x < 8; x++)
		{
			if (CheckPositionIsInTransform(tile[y][x].transform, MousePosition(lParam)))
				return &tile[y][x];
		}
	}
	return NULL;
...
void Chess::SetCursorPosition(Position position)
{
